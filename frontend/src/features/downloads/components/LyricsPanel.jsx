...
const LyricsPanel = ({ visible, onClose, baseUrl, albumId, track }) => {
  ...
  useEffect(() => {
    ...
    async function fetchLyrics() {
      ...
      setLyrics(null);
      try {
        const endpoint = baseUrl ? `${baseUrl}/api/items/${albumId}/lyrics` : endpoints.items.lyrics(albumId);
        const response = await get(endpoint, { params: { title, artist } });
        if (!cancelled) {
          setLyrics(response?.lyrics || null);
        }
      } catch (err) {
        if (!cancelled) {
          setError(
