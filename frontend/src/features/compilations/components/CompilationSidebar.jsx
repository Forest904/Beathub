...
const SidebarContent = () => {
  ...
  const handleStartDownload = async () => {
    ...
    try {
      ...
      const res = await post(endpoints.compilations.download(), payload);
      const compId = res?.compilation_spotify_id;
      clear();
      ...
      <aside className="absolute inset-y-0 left-0 w-[22rem] max-w-[90vw] bg-white p-4 shadow-xl dark:bg-slate-900 flex flex-col">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold">
            <span className="inline-block rounded-md px-2 py-1 text-black dark:text-white">Make your own compilation</span>
          </h2>
          <button
            type="button"
        ...
        <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-md px-3 py-1.5 text-sm font-medium bg-slate-200 text-slate-700 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-200 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed dark:disabled:bg-slate-800 dark:disabled:text-slate-600"
            >
              Confirm Compilation
            </button>
            <button
              ...
              className="rounded-md px-3 py-1.5 text-sm font-medium bg-brand-600 text-white hover:bg-brand-700 dark:bg-brandDark-600 dark:hover:bg-brandDark-500 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed dark:disabled:bg-slate-800 dark:disabled:text-slate-600"
            >
