...
import { fetchArtists, toArtistSummary } from '../../../api/mappers';
...
const ArtistBrowserPage = () => {
  ...
  const loadFamousArtists = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await get(endpoints.artists.popular(), {
        params: { limit: POPULAR_LIMIT, page, order_by: orderBy, order_dir: 'desc' },
      }) || {};
      const list = Array.isArray(data.artists) ? data.artists.map(toArtistSummary) : [];
      setFamousArtists(list);
  ...
  useEffect(() => {
    const searchArtists = async () => {
      ...
      setError(null);
      try {
        const { artists: list, pagination } = await fetchArtists({ q: debouncedSearchTerm, page, limit: POPULAR_LIMIT });
        setArtists(list);
        const p = pagination || {};
        setPageInfo({
          page: p.page || page,
