        ...
        <div className="p-4 flex flex-col flex-grow gap-2">
          ...
          {(hasFollowers || hasPopularity) && (
            <div className="flex flex-wrap items-center gap-1">
              {hasFollowers && (
                <span className="inline-flex max-w-full flex-wrap items-center gap-1 rounded-full border border-brand-100 bg-brand-50 px-2 py-0.5 text-[0.7rem] font-semibold text-brand-700 dark:border-brandDark-400 dark:bg-brandDark-500/20 dark:text-brandDark-100">
                  <span className="uppercase tracking-wide text-[0.65rem] leading-tight">Followers</span>
              ...
              {hasPopularity && (
                <span className="inline-flex max-w-full flex-wrap items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[0.7rem] font-semibold text-slate-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100">
                  <span className="uppercase tracking-wide text-[0.65rem] leading-tight">Popularity</span>
                  <span className="text-xs font-bold leading-tight">
                    {artist.popularity}
                  </span>
                </span>
          ...
          {artist.genres?.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {artist.genres.slice(0, 4).map((genre) => (
                <span
                  key={genre}
                  className="inline-flex max-w-full flex-wrap items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[0.7rem] font-semibold text-slate-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 break-words text-left"
                >
                  {genre.charAt(0).toUpperCase() + genre.slice(1)}
                </span>
              ))}
              {artist.genres.length > 4 && (
                <span className="inline-flex max-w-full flex-wrap items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[0.7rem] font-semibold text-slate-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 break-words text-left">
                  +{artist.genres.length - 4}
                </span>
              )}
            </div>
