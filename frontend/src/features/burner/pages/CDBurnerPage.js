...
const CDBurnerPage = () => {
  ...
  const apiBaseUrl = API_BASE_URL || 'http://localhost:5000';
  ...
  const pollBurnerStatus = useCallback(async () => {
    ...
    try {
      ...
      if (response?.last_error && (burnerStatus.is_burning || isBurningInitiating)) {
        pushMessage({ type: 'error', text: `Burner error: ${response.last_error}` });
      }
    } catch (error) {
  ...
  const fetchDevices = useCallback(
    async (options = { showSpinner: false }) => {
      ...
      try {
        const response = await get(endpoints.burner.devices());
        setDevices(response.devices || []);
        if (response?.error) {
          pushMessage({ type: 'error', text: response.error });
        }
      } catch (error) {
  ...
  const handleBurnCD = useCallback(async () => {
    ...
    try {
      await post(endpoints.burner.burn(), { download_item_id: selectedItem.id });
  ...
  const handleCancelBurn = useCallback(async () => {
    ...
    try {
      await post(endpoints.burner.cancel(), { session_id: activeSessionId });
      pollBurnerStatus();
    } catch (error) {
  ...
  const fetchPreview = useCallback(async () => {
    ...
    try {
      const res = await post(endpoints.burner.preview(), {
