import sys
import types
from pathlib import Path

import pytest


def _install_spotdl_stubs(monkeypatch, print_on_download: bool = False):
    """Install stub modules for 'spotdl' used by SpotdlClient's engine thread.

    Returns the created module so tests can tweak behavior if needed.
    """
    m_spotdl = types.ModuleType("spotdl")
    m_types = types.ModuleType("spotdl.types")
    m_options = types.ModuleType("spotdl.types.options")

    class DummyProgressHandler:
        def __init__(self):
            self.update_callback = None
            self.web_ui = False

    class DummyDownloader:
        def __init__(self):
            self.settings = {"output": None}
            self.progress_handler = DummyProgressHandler()

    class DummySong:
        def __init__(self, display_name: str = "S"):
            self.display_name = display_name

    class DummySpotdl:
        def __init__(self, client_id=None, client_secret=None, downloader_settings=None):
            self.downloader = DummyDownloader()
            self._songs = [DummySong("A"), DummySong("B")]

        def search(self, queries):
            return self._songs

        def download_songs(self, songs):
            if print_on_download:
                # Intentionally print from inside the engine thread
                print("ENGINE: starting download")
                print("ENGINE: progress 50%")
            return [(s, Path(f"/tmp/{s.display_name}.mp3")) for s in songs]

    # Also provide a DownloaderOptionalOptions stub to avoid downstream import issues in other tests
    class DummyDownloaderOptionalOptions:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)

    m_options.DownloaderOptionalOptions = DummyDownloaderOptionalOptions

    m_spotdl.Spotdl = DummySpotdl
    monkeypatch.setitem(sys.modules, "spotdl", m_spotdl)
    monkeypatch.setitem(sys.modules, "spotdl.types", m_types)
    monkeypatch.setitem(sys.modules, "spotdl.types.options", m_options)
    return m_spotdl


@pytest.mark.unit
def test_engine_init_failure_without_spotdl_class(monkeypatch):
    # Ensure the engine thread cannot import a valid Spotdl class even if the
    # real package is installed. Provide a module without the attribute and
    # restore modules after test via monkeypatch.
    for k in list(sys.modules.keys()):
        if k == "spotdl" or k.startswith("spotdl."):
            monkeypatch.delitem(sys.modules, k, raising=False)
    monkeypatch.setitem(sys.modules, "spotdl", types.ModuleType("spotdl"))

    from src.spotdl_client import SpotdlClient

    with pytest.raises(RuntimeError, match="Failed to initialize SpotDL engine thread"):
        SpotdlClient(client_id="id", client_secret="sec", downloader_options={})


@pytest.mark.unit
def test_set_output_template_and_clear_progress_callback(monkeypatch):
    _install_spotdl_stubs(monkeypatch)
    from src.spotdl_client import SpotdlClient

    cli = SpotdlClient(client_id="id", client_secret="sec", downloader_options={})

    eff = cli.set_output_template("{artist}-{title}.{ext}")
    assert eff == "{artist}-{title}.{ext}"
    assert cli.spotdl.downloader.settings["output"] == "{artist}-{title}.{ext}"

    # Attach progress callback then clear it
    events = []

    def cb(ev: dict):
        events.append(ev)

    cli.set_progress_callback(cb, web_ui=True)
    ph = cli.spotdl.downloader.progress_handler
    assert ph.web_ui is True
    assert callable(ph.update_callback)

    # Now clear and verify
    cli.clear_progress_callback()
    assert cli.spotdl.downloader.progress_handler.update_callback is None


@pytest.mark.unit
def test_progress_callback_wrapper_handles_exceptions(monkeypatch):
    _install_spotdl_stubs(monkeypatch)
    from src.spotdl_client import SpotdlClient

    cli = SpotdlClient(client_id="id", client_secret="sec", downloader_options={})

    # Callback that raises; wrapper should swallow and not raise
    def raising_cb(ev: dict):
        raise RuntimeError("boom")

    cli.set_progress_callback(raising_cb, web_ui=True)
    ph = cli.spotdl.downloader.progress_handler

    # Make a dummy tracker object with parent fields
    Parent = type("P", (), {"overall_completed_tasks": 1, "song_count": 2, "overall_progress": 50})
    Tracker = type("T", (), {"song_name": "S", "progress": 42, "parent": Parent()})

    # Should not raise
    ph.update_callback(Tracker(), "Downloading")


@pytest.mark.unit
def test_download_songs_silences_stdout(monkeypatch, capsys):
    _install_spotdl_stubs(monkeypatch, print_on_download=True)
    from src.spotdl_client import SpotdlClient

    cli = SpotdlClient(client_id="id", client_secret="sec", downloader_options={})
    songs = cli.search(["https://open.spotify.com/track/xyz"])  # via stub

    # Call and capture output from the test thread
    res = cli.download_songs(songs)
    out, err = capsys.readouterr()

    # Ensure stub prints were silenced by the client
    assert "ENGINE:" not in out
    assert len(res) == 2
    assert str(res[0][1]).endswith("A.mp3")


@pytest.mark.unit
def test_download_link_orchestrates_search_download_and_progress(monkeypatch):
    _install_spotdl_stubs(monkeypatch)
    from src.spotdl_client import SpotdlClient

    cli = SpotdlClient(client_id="id", client_secret="sec", downloader_options={})

    seen = []

    def cb(ev: dict):
        seen.append(ev)

    # Attach callback; download_link sets output and callback, runs search+download
    res = cli.download_link("https://open.spotify.com/track/xyz", "{title}.{ext}", progress_callback=cb)

    assert len(res) == 2
    assert cli.spotdl.downloader.settings["output"] == "{title}.{ext}"

    # Simulate one progress update
    Parent = type("P", (), {"overall_completed_tasks": 1, "song_count": 2, "overall_progress": 50})
    Tracker = type("T", (), {"song_name": "A", "progress": 10, "parent": Parent()})
    ph = cli.spotdl.downloader.progress_handler
    ph.update_callback(Tracker(), "Downloading")

    assert seen, "expected progress callback to be invoked"
    assert seen[-1]["song_display_name"] == "A"
